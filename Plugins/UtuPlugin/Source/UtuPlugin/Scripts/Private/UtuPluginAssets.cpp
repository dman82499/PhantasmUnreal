...
{
	// Format Paths
	TArray<FString> AssetNames = StartProcessAsset(InUtuAnimation);
	...
	if (DeleteInvalidAssetIfNeeded(AssetNames_Anim, UAnimSequence::StaticClass()) && DeleteInvalidAssetIfNeeded(AssetNames_Anim, UAnimSequence::StaticClass())) {
		...
		// UFbxAssetImportData
		Options->AnimSequenceImportData->ImportTranslation = ImportSettings.AnimationImportLocationOffset;
		Options->AnimSequenceImportData->ImportRotation = ImportSettings.AnimationImportRotationOffset;
		Options->AnimSequenceImportData->ImportUniformScale = ImportSettings.AnimationImportScaleMultiplier;
		Options->AnimSequenceImportData->bConvertScene = ImportSettings.bConvertScene;
		Options->AnimSequenceImportData->bForceFrontXAxis = ImportSettings.bForceFrontXAxis;
		Options->AnimSequenceImportData->bConvertSceneUnit = ImportSettings.bConvertSceneUnit;
		// UFbxAnimSequenceImportData
		Options->AnimSequenceImportData->bImportMeshesInBoneHierarchy = ImportSettings.bAnimImportMeshesInBoneHierarchy;
		...
		Options->AnimSequenceImportData->bUseDefaultSampleRate = ImportSettings.bUseDefaultSampleRate;
		Options->AnimSequenceImportData->CustomSampleRate = ImportSettings.CustomSampleRate;
		Options->AnimSequenceImportData->bImportCustomAttribute = ImportSettings.bImportCustomAttribute;
		Options->AnimSequenceImportData->bDeleteExistingCustomAttributeCurves = ImportSettings.bDeleteExistingCustomAttributeCurves;
		Options->AnimSequenceImportData->bSetMaterialDriveParameterOnCustomAttribute = ImportSettings.bSetMaterialDriveParameterOnCustomAttribute;
		Options->AnimSequenceImportData->bRemoveRedundantKeys = ImportSettings.bRemoveRedundantKeys;
		Options->AnimSequenceImportData->bDeleteExistingMorphTargetCurves = ImportSettings.bDeleteExistingMorphTargetCurves;
		Options->AnimSequenceImportData->bDoNotImportCurveWithZero = ImportSettings.bDoNotImportCurveWithZero;
		Options->AnimSequenceImportData->bPreserveLocalTransform = ImportSettings.bPreserveLocalTransform;
		...
		{
			TArray<FString> SkeletonAssetNames = FormatRelativeFilenameForUnreal(SkeletalMeshes, false);
	...
	if (InUtuMesh.asset_relative_filename.StartsWith("Assets")) { // Default Unity Mesh
		...
		if (InUtuMesh.is_skeletal_mesh) {
			...
			if (DeleteInvalidAssetIfNeeded(AssetNames, USkeletalMesh::StaticClass())) {
				...
				Options->SkeletalMeshImportData->ImportTranslation = UtuConst::ConvertLocation(InUtuMesh.mesh_import_position_offset, true, InUtuMesh.mesh_import_rotation_offset) / FMath::Max(0.0001f, InUtuMesh.mesh_import_scale_offset.X) + ImportSettings.SkeletalMeshImportLocationOffset;
				Options->SkeletalMeshImportData->ImportRotation = UKismetMathLibrary::ComposeRotators(ImportSettings.SkeletalMeshImportRotationOffset, FRotator(UtuConst::ConvertRotation(InUtuMesh.mesh_import_rotation_offset, true)));
				Options->SkeletalMeshImportData->ImportUniformScale = ImportSettings.SkeletalMeshImportScaleMultiplier * InUtuMesh.mesh_import_scale_factor / FMath::Max(0.0001f, InUtuMesh.mesh_import_scale_offset.X);
		...
		else 
		{
			...
			if (DeleteInvalidAssetIfNeeded(AssetNames, UStaticMesh::StaticClass())) 
			{
				...
				Options->StaticMeshImportData->ImportTranslation = UtuConst::ConvertLocation(InUtuMesh.mesh_import_position_offset, true, InUtuMesh.mesh_import_rotation_offset) / FMath::Max(0.0001f, InUtuMesh.mesh_import_scale_offset.X) + ImportSettings.StaticMeshImportLocationOffset;
				Options->StaticMeshImportData->ImportRotation = UKismetMathLibrary::ComposeRotators(ImportSettings.StaticMeshImportRotationOffset, FRotator(UtuConst::ConvertRotation(InUtuMesh.mesh_import_rotation_offset, true)));
				...
				else
				{
					...
					{
						TArray<FString> SubMeshAssetNames = TArray<FString>();
						SubMeshAssetNames.Add(AssetNames[0]);
						SubMeshAssetNames.Add(AssetNames[1] + "_" + SubMesh.submesh_name.Replace(TEXT(" "), TEXT("_")).Replace(TEXT(".fbx"), TEXT("")));
						SubMeshAssetNames.Add(SubMeshAssetNames[0] + "/" + SubMeshAssetNames[1]);
						Asset = GetMeshAsset(SubMeshAssetNames);
						...
						if (Asset != nullptr)
						{
							...
							UTU_LOG_L("                Associating Materials to Static Mesh...");
							for (int x = 0; x < SubMesh.submesh_materials_relative_filenames.Num(); x++)
							{
								TArray<FString> MatNames = FormatRelativeFilenameForUnreal(SubMesh.submesh_materials_relative_filenames[x], true);
								UTU_LOG_L("                    MaterialId[" + FString::FromInt(x) + "] : " + MatNames[2]);
								UMaterialInterface* MaterialAsset = Cast<UMaterialInterface>(UUtuPluginLibrary::TryGetAsset(MatNames[2]));
								Asset->SetMaterial(x, MaterialAsset);
								if (MaterialAsset == nullptr)
...
UMaterialExpressionTextureSampleParameter2D* FUtuPluginAssetTypeProcessor::GetOrCreateTextureParameter(UMaterial* InMaterial, UTexture* InTexture, FName InParamName, int InPosX, int InPosY, FUtuPluginMaterial InUtuMaterial) {
	...
	if (InMaterial != nullptr) {
		...
		if (Ret != nullptr) {
			...
			{
				InTexture = Cast<UTexture2D>(UUtuPluginLibrary::TryGetAsset("/UtuPlugin/Default/Texture"));
			}
...
AActor* FUtuPluginAssetTypeProcessor::WorldSpawnStaticMeshActor(UWorld* InAsset, FUtuPluginActor InUtuActor) {
	...
	if (RetActor != nullptr) {
		...
		UStaticMesh* StaticMeshAsset = GetMeshAsset(MeshNames);
		if (StaticMeshAsset == nullptr && ImportSettings.bImportSeparated) {
			StaticMeshAsset = GetMeshAsset(MeshNamesSeparated);
		}
